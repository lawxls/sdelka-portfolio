...
5. [Rendering Performance](#5-rendering-performance) — **MEDIUM**
   ...
   - 5.8 [Use defer or async on Script Tags](#58-use-defer-or-async-on-script-tags)
   - 5.9 [Use Explicit Conditional Rendering](#59-use-explicit-conditional-rendering)
   - 5.10 [Use React DOM Resource Hints](#510-use-react-dom-resource-hints)
   - 5.11 [Use useTransition Over Manual Loading States](#511-use-usetransition-over-manual-loading-states)
6. [JavaScript Performance](#6-javascript-performance) — **LOW-MEDIUM**
   ...
   - 6.10 [Use flatMap to Map and Filter in One Pass](#610-use-flatmap-to-map-and-filter-in-one-pass)
   - 6.11 [Use Loop for Min/Max Instead of Sort](#611-use-loop-for-minmax-instead-of-sort)
   - 6.12 [Use Set/Map for O(1) Lookups](#612-use-setmap-for-o1-lookups)
   - 6.13 [Use toSorted() Instead of sort() for Immutability](#613-use-tosorted-instead-of-sort-for-immutability)
...
In SSR setups, some values are intentionally different on server vs client (random IDs, dates, locale/timezone formatting). For these *expected* mismatches, wrap the dynamic text in an element with `suppressHydrationWarning` to prevent noisy warnings. Do not use this to hide real bugs. Don’t overuse it.
...
### 5.8 Use defer or async on Script Tags
...
### 5.9 Use Explicit Conditional Rendering
...
### 5.10 Use React DOM Resource Hints
...
### 5.11 Use useTransition Over Manual Loading States
...
### 6.10 Use flatMap to Map and Filter in One Pass
...
### 6.11 Use Loop for Min/Max Instead of Sort
...
### 6.12 Use Set/Map for O(1) Lookups
...
### 6.13 Use toSorted() Instead of sort() for Immutability
