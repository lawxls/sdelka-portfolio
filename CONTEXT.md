...
- **ProcurementInquiry** (Тендер) — primary procurement container that
  bundles a 1:N collection of `ProcurementItem`s sharing one budget,
  deadline, company, and category. Slug `id` (e.g. `T-001`, generated by
  `generateTenderSlug`) doubles as URL param at `/tenders/:slug`. Tender
  status is a pure rollup of item statuses via `getTenderStatus` (no
  separate stored status). Backs `useTenders`. The mock heuristic that
  groups parsed positions into proposed tenders (slice #9) lives at
  `tenders/group-items-into-tenders.ts`.
