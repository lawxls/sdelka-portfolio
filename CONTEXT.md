...
- **In-memory adapter** — implements a `DataClient` against a mock store.
  ...
  `createInMemoryFoldersClient({ seed })`. Companies' adapter is
  closure-isolated. Items', suppliers', tasks', and folders' adapters wrap
  the module-level singletons (`items-mock-data`, `supplier-mock-data`,
  `tasks-mock-data`, `folders-mock-data`) so cross-entity callers see the
  same store the hook sees — folders' delete reaches into items via
...
- **DataClientsProvider** — React context that holds the `DataClients` map.
  ...
  `useTasksClient()`, `useFoldersClient()` (and future friends) read from
...
- **CursorPage<T>** — shared list shape `{ items, nextCursor }` for cursor-
  ...
  snapshot keyed by folder id with an `archiveCount`.
