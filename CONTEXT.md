...
- **ProcurementInquiry** (Тендер) — primary procurement container that
  bundles a 1:N collection of `ProcurementItem`s sharing one deadline,
  company, and category. Slug `id` (e.g. `T-001`, generated by
  `generateTenderSlug`) doubles as URL param at `/tenders/:slug`. Status
  is stored on the backend (`searching` | `searching_completed` |
  `negotiating` | `completed`) — no client-side rollup from item statuses.
  Counts (`kpCount`, `positionsCount`, `tasksCount`, `suppliersCount`) are
  server-side annotations on the list/retrieve queryset. Backs
  `useProcurementInquiries`. The mock heuristic that groups parsed
  positions into proposed inquiries (slice #9) lives at
  `procurement-inquiries/group-items-into-procurement-inquiries.ts`.

  **In-the-meantime cross-domain limitation:** the production composition
  root wires `procurementInquiries` to the HTTP adapter unconditionally,
  but items still run through the in-memory adapter until the items HTTP
  integration lands. `createProcurementInquiryWithItems` therefore mixes
  an HTTP inquiry create with mocked item creates — the inquiry survives
  a reload, its items don't. Tests that need the cross-entity cascade
  (items hidden when the parent inquiry is archived, items inheriting
  inquiry's folder/company) register a resolver via
  `_setInquiryStateResolver` (`src/data/items-mock-data.ts`).
