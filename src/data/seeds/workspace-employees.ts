import type { WorkspaceEmployeeDetail } from "../domains/workspace-employees";

/**
 * Default workspace-employees roster — admin + 8 others, spanning roles and
 * registration states. Companies are intentionally empty: a fresh workspace
 * has no companies, and the company-chip on each row is populated by the
 * composition-root callback that resolves `CompanySummary[]` from the active
 * companies adapter. Test fixtures that need a company chip seed it directly.
 */
export const SEED_WORKSPACE_EMPLOYEES: WorkspaceEmployeeDetail[] = [
	{
		id: "1",
		firstName: "Иван",
		lastName: "Журавлёв",
		patronymic: "Сергеевич",
		position: "Директор по закупкам",
		role: "admin",
		phone: "+79161000001",
		email: "ivan.zhuravlyov.58@mostholding.ru",
		registeredAt: "2024-01-15T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-1",
			employeeId: "1",
			procurementInquiries: "edit",
			positions: "edit",
			tasks: "edit",
			workspaceSettings: "edit",
			companies: "edit",
			employees: "edit",
			emails: "edit",
		},
	},
	{
		id: "2",
		firstName: "Ольга",
		lastName: "Соколова",
		patronymic: "Андреевна",
		position: "Руководитель отдела закупок",
		role: "admin",
		phone: "+79161000002",
		email: "o.sokolova@ormatek.com",
		registeredAt: "2024-02-01T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-2",
			employeeId: "2",
			procurementInquiries: "edit",
			positions: "edit",
			tasks: "edit",
			workspaceSettings: "edit",
			companies: "edit",
			employees: "edit",
			emails: "edit",
		},
	},
	{
		id: "3",
		firstName: "Дмитрий",
		lastName: "Орлов",
		patronymic: "Михайлович",
		position: "Менеджер по закупкам",
		role: "user",
		phone: "+79161000003",
		email: "d.orlov@ormatek.com",
		registeredAt: "2024-03-12T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-3",
			employeeId: "3",
			procurementInquiries: "edit",
			positions: "edit",
			tasks: "edit",
			workspaceSettings: "view",
			companies: "view",
			employees: "view",
			emails: "view",
		},
	},
	{
		id: "4",
		firstName: "Екатерина",
		lastName: "Белова",
		patronymic: "Игоревна",
		position: "Аналитик отдела закупок",
		role: "user",
		phone: "+79161000004",
		email: "e.belova@ormatek.com",
		registeredAt: "2024-05-20T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-4",
			employeeId: "4",
			procurementInquiries: "view",
			positions: "view",
			tasks: "view",
			workspaceSettings: "none",
			companies: "view",
			employees: "none",
			emails: "none",
		},
	},
	{
		id: "5",
		firstName: "Николай",
		lastName: "Фёдоров",
		patronymic: "Олегович",
		position: "Категорийный менеджер",
		role: "user",
		phone: "+79161000005",
		email: "n.fedorov@ormatek.com",
		registeredAt: "2024-08-03T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-5",
			employeeId: "5",
			procurementInquiries: "edit",
			positions: "edit",
			tasks: "edit",
			workspaceSettings: "view",
			companies: "view",
			employees: "none",
			emails: "view",
		},
	},
	{
		id: "6",
		firstName: "Анна",
		lastName: "Смирнова",
		patronymic: "Николаевна",
		position: "Финансовый аналитик",
		role: "user",
		phone: "+79161000006",
		email: "a.smirnova@ormatek.com",
		registeredAt: "2025-01-10T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-6",
			employeeId: "6",
			procurementInquiries: "view",
			positions: "view",
			tasks: "view",
			workspaceSettings: "none",
			companies: "none",
			employees: "none",
			emails: "none",
		},
	},
	{
		id: "7",
		firstName: "Дмитрий",
		lastName: "Попов",
		patronymic: "",
		position: "Менеджер по закупкам",
		role: "user",
		phone: "",
		email: "d.popov@ormatek.com",
		registeredAt: null,
		companies: [],
		permissions: {
			id: "perm-w-7",
			employeeId: "7",
			procurementInquiries: "none",
			positions: "none",
			tasks: "none",
			workspaceSettings: "none",
			companies: "none",
			employees: "none",
			emails: "none",
		},
	},
	{
		id: "8",
		firstName: "Павел",
		lastName: "Кузнецов",
		patronymic: "Викторович",
		position: "Старший специалист по закупкам",
		role: "user",
		phone: "+79161000008",
		email: "p.kuznetsov@ormatek.com",
		registeredAt: "2025-03-04T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-8",
			employeeId: "8",
			procurementInquiries: "edit",
			positions: "edit",
			tasks: "view",
			workspaceSettings: "view",
			companies: "view",
			employees: "view",
			emails: "edit",
		},
	},
	{
		id: "9",
		firstName: "Мария",
		lastName: "Лебедева",
		patronymic: "Дмитриевна",
		position: "Специалист по тендерам",
		role: "user",
		phone: "+79161000009",
		email: "m.lebedeva@ormatek.com",
		registeredAt: "2025-09-22T10:00:00Z",
		companies: [],
		permissions: {
			id: "perm-w-9",
			employeeId: "9",
			procurementInquiries: "view",
			positions: "edit",
			tasks: "edit",
			workspaceSettings: "none",
			companies: "view",
			employees: "none",
			emails: "view",
		},
	},
];
