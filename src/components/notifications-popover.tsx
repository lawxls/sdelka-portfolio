...
function resolveNotification(
	...
	lookups: {
		...
		taskName: (id: string) => { name: string; procurementInquiryName: string } | undefined;
	...
	switch (n.type) {
		...
		case "task_deadline_24h": {
			...
			return {
				notification: n,
				title,
				context: task ? `${task.name} · ${task.procurementInquiryName}` : null,
...
export function NotificationsPopover() {
	...
	const lookups = useMemo(() => {
		...
		const taskMap = new Map(
			(tasksQ.data ?? []).map((t) => [t.id, { name: t.name, procurementInquiryName: t.procurementInquiry.name }]),
		);
