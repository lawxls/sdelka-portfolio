...
export function ImportItemCard({ item, index }: ImportItemCardProps) {
	...
	const hasProcurement = item.procurementType != null;
	const hasPayment = item.paymentType != null || item.vatIncluded != null || item.paymentMethod != null;
			...
			<div className="mt-3 flex flex-col gap-2">
				{(hasBasicInfo || hasProcurement) && (
					<dl className="grid grid-cols-2 gap-x-4 gap-y-1">
						...
						{item.procurementType != null && (
							<Field label="Тип закупки" value={PROCUREMENT_TYPE_LABELS[item.procurementType]} />
						)}
						{item.procurementType === "regular" && item.frequency != null && (
							<Field label="Периодичность" value={FREQUENCY_LABELS[item.frequency]} />
						)}
					</dl>
				)}

				{item.legalEntityMode != null && (
					<Section title="Юр. лицо">
						<Field label="Режим" value={LEGAL_ENTITY_LABELS[item.legalEntityMode]} />
						{item.legalEntityMode === "company" && item.legalEntityCompany && (
							<Field label="Компания" value={item.legalEntityCompany} />
						)}
					</Section>
				)}
				...
				{hasPayment && (
					<Section title="Оплата">
						...
						{item.vatIncluded != null && <Field label="НДС" value={item.vatIncluded ? "С НДС" : "Без НДС"} />}
