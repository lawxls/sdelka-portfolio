...
describe("ProcurementPage", () => {
	...
	test("«Создать запрос» on /inquiries opens the create-procurement-inquiry drawer and submitting persists the procurementInquiry + items", async () => {
		...
		// Inquiry name is server-generated by the inquiry-name LLM seam; the
		// in-memory client mirrors the BE's mock backend by prefixing the
		// first position name with «Запрос: ».
		await waitFor(() => {
			expect(screen.getByText("Запрос: Позиция А")).toBeInTheDocument();
		});
	});
	...
	test("file import via dialog dispatches a inquiry per AI-grouped batch and closes the dialog", async () => {
		...
		// Two AI-grouped inquiries should land on /inquiries: «Кабель ВВГнг 3x2.5»
		// (group of 2) and «Розетка серая» (group of 1). Inquiry names are
		// server-generated; the in-memory client mirrors the BE's mock backend
		// by prefixing the first item's name with «Запрос: ».
		const rail = screen.getByTestId("app-rail");
		await user.click(within(rail).getByRole("button", { name: /Запросы/ }));
		await waitFor(() => {
			expect(screen.getByText("Запрос: Кабель ВВГнг 3x2.5")).toBeInTheDocument();
			expect(screen.getByText("Запрос: Розетка серая")).toBeInTheDocument();
		});
	});
});
